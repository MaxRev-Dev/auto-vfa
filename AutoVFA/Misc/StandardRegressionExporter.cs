...
namespace AutoVFA.Misc
{
    ...
    {
        public StandardRegressionExporter(AnalyzingContext context)
        {
        ...
        public override async Task ExportToXLSX(string fileName)
        {
            try
            {
                ...
                {
                    RegressionResult result = _results[i];
                    worksheet.Cells[offsetX + i + 2, offsetY + 1].Value =
                        result.Acid;
                    worksheet.Cells[offsetX + i + 2, offsetY + 2].Value =
                        result.A;
                    worksheet.Cells[offsetX + i + 2, offsetY + 3].Value =
                        result.B;
                    worksheet.Cells[offsetX + i + 2, offsetY + 4].Value =
                        result.R2;
                }
                ...
                {
                    worksheet.Cells[offsetX - 2, offsetY + 2 + i].Value =
                        _standards[i].Name;
                    worksheet.Cells[offsetX - 1, offsetY + 2 + i].Value =
                        $"Level {i + 1}";
                }

                var rowPad = 2;
                worksheet.Cells[offsetX + rowPad + acids.Length - 1,
                        offsetY + 1].Value =
                    $"Norm-d with int. standard ({_baseNormAcid})";
                using (ExcelRange range = worksheet.Cells[
                    offsetX + rowPad + acids.Length - 1, offsetY + 1,
                    offsetX + rowPad + acids.Length - 1,
                    offsetY + _standards.Count + 1])
                {
                    range.Merge = true;
                ...
                foreach (var acid in acids.Except(new[] {_baseNormAcid}))
                {
                    ...
                    worksheet.Cells[offsetX + rowPad + acids.Length,
                        offsetY + 1].Value = acid;

                    for (var i = 0; i < vX.Length; i++)
                    {
                        worksheet.Cells[offsetX, offsetY + 2 + i].Value = vX[i];
                        worksheet.Cells[offsetX + rowPad + acids.Length,
                            offsetY + 2 + i].Value = vY[i];
                    }

                    ExcelScatterChart scatterChart = worksheet.Drawings
                        .AddChartFromTemplate(
                            new FileInfo("Templates/template.crtx"), acid)
                        .As.Chart.ScatterChart;
                    scatterChart.Title.Text = acid;
                    scatterChart.SetPosition(chartOffsetX, 0,
                        chart++ * chartWidth, 0);
                    scatterChart.To.Row = scatterChart.From.Row + chartHeight;
                    scatterChart.To.Column =
                        scatterChart.From.Column + chartWidth - 1;
                    ExcelScatterChartSerie serie = scatterChart.Series.Add(
                        worksheet.Cells[offsetX + rowPad + acids.Length,
                            offsetY + 2, offsetX + rowPad + acids.Length,
                            offsetY + 2 + vX.Length - 1],
                        worksheet.Cells[offsetX, offsetY + 2, offsetX,
                            offsetY + 2 + vX.Length - 1]);
        ...
        public override Task ExportToCsv(string fileName)
        {
            return _results.ExportToCSVFileAsync(fileName);
        }
    }
}